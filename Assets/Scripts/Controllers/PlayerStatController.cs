...
public class PlayerStatController : MonoBehaviour
{
    ...
    public List<PlayerStat> moveSpeed, health, pickupRange, maxWeapons;
    public int moveSpeedCount, healthCount, pickupRangeCount;
    ...
    void Start()
    {
        ...
        {
            moveSpeed.Add(new PlayerStat(moveSpeed[i].cost + moveSpeed[1].cost, moveSpeed[i].value + (moveSpeed[1].value - moveSpeed[0].value)));
        }

        for (int i = health.Count - 1; i < healthCount; i++)
        {
            health.Add(new PlayerStat(health[i].cost + health[1].cost, health[i].value + (health[1].value - health[0].value)));
        }

        for (int i = pickupRange.Count - 1; i < pickupRangeCount; i++)
        {
            pickupRange.Add(new PlayerStat(pickupRange[i].cost + pickupRange[1].cost, pickupRange[i].value + (pickupRange[1].value - pickupRange[0].value)));
        }
    }
    ...
    public void UpdateDisPlay()
    {
        ...
        {
            UIController.instance.pickupRangeDisplay.UpdateDisplay(pickupRange[pickupRangeLevel + 1].cost, pickupRange[pickupRangeLevel].value, pickupRange[pickupRangeLevel + 1].value);
        }
        else
        {
            UIController.instance.pickupRangeDisplay.ShowMaxLevel();
        }
...
public class PlayerStat
{
    ...
    public PlayerStat(int newCost, float newValue)
    {
        cost = newCost; value = newValue;
    }
}
